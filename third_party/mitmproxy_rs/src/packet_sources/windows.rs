...
impl PacketSourceConf for WindowsConf {
    ...
    async fn build(
        ...
        if result.0 as u32 == SE_ERR_ACCESSDENIED as u32 {
            return Err(anyhow!(
                "Network capture was cancelled or denied. Approve the Administrator prompt, \
                 or use --no-proxy to inject secrets without OS capture."
            ));
        } else if result.0 as u32 <= 32 {
            let err = windows::core::Error::from_win32();
            return Err(anyhow!("Failed to start the redirector as administrator: {err}"));
        }
