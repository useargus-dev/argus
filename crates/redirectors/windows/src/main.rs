...
fn init_logging() {
    let capture = capture_log::enabled();
    if cfg!(debug_assertions) || capture {
        env_logger::Builder::from_env(env_logger::Env::default().default_filter_or("info")).init();
    }
    if capture {
        capture_log::log("redirector", "WinDivert redirector trace on (stderr)");
    }
}
        ...
        ConnectionAction::Intercept(ProcessInfo { pid, process_name }) => {
            ...
            if payload_len > 0 {
                capture_log::log(
                    "redirector",
                    format!(
                        "IPC→relay pid={pid} {} {} payload={payload_len}B",
                        packet.connection_id(),
                        packet.tcp_flag_str()
                    ),
                );
