...
pub fn create_session(
    ...
    conn.execute(
        "INSERT INTO sandbox_sessions
         (id, bucket_id, grant_id, parent_fingerprint, command_preview, created_at, expires_at)
...
pub fn register_pids(conn: &Connection, session_id: &str, pids: &[u32]) -> AppResult<()> {
    if pids.is_empty() {
        return Ok(());
    }
    ...
    for pid in pids {
        conn.execute(
            "INSERT INTO sandbox_session_pids (session_id, pid, added_at)
             VALUES (?1, ?2, ?3)
             ON CONFLICT(session_id, pid) DO NOTHING",
            params![session_id, *pid as i64, now],
        )
        .map_err(|e| AppError::message("DB_ERROR", e.to_string()))?;
    }
...
pub fn revoke_session(conn: &Connection, session_id: &str) -> AppResult<bool> {
    ...
    let n = conn
        .execute(
            "UPDATE sandbox_sessions SET revoked_at = ?2
...
pub fn get_session(conn: &Connection, session_id: &str) -> AppResult<Option<SandboxSession>> {
    let row = conn.query_row(
        "SELECT id, bucket_id, grant_id, parent_fingerprint, command_preview, root_pid,
...
pub fn lookup_active_session_by_pid(
    ...
    let row = conn.query_row(
        "SELECT s.id, s.bucket_id, s.grant_id, s.parent_fingerprint, s.command_preview,
                s.root_pid, s.created_at, s.expires_at
         FROM sandbox_sessions s
         INNER JOIN sandbox_session_pids p ON p.session_id = s.id
         WHERE p.pid = ?1
...
mod tests {
    ...
    fn session_create_register_lookup_revoke() {
        ...
        .unwrap();
        register_pids(&conn, &session.id, &[1234, 5678]).unwrap();
